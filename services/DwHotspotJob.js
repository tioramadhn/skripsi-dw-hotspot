class DwHotspotJob {
  ...
  async updateDimTime() {
        ...
        SELECT distinct (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date AS id_time,
        EXTRACT(YEAR FROM (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date) AS tahun,
        CASE
          WHEN EXTRACT(MONTH FROM (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date) <= 6 THEN 1
          ELSE 2
        END AS semester,
        CONCAT('Q', CEIL(EXTRACT(MONTH FROM (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date) / 3.0)) AS kuartal,
        CASE EXTRACT(MONTH FROM (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date)
          WHEN 1 THEN 'Januari'
        ...
        END AS bulan,
        CASE
          WHEN EXTRACT(WEEK FROM (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date) <= 4 THEN EXTRACT(WEEK FROM (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date)
          ELSE 4
        END AS minggu,
        CASE EXTRACT(DOW FROM (h.hs_id::timestamp AT TIME ZONE 'UTC+7')::date)
          WHEN 0 THEN 'Minggu'
          WHEN 1 THEN 'Senin'
          WHEN 2 THEN 'Selasa'
          WHEN 3 THEN 'Rabu'
